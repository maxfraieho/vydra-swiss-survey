...
> [!NOTE]
> Це **додаток** до основного плану self-learning playbook (фази 1–8; фази 1–5
> вже злиті: `host_rules`/`run_traces` схема, `reflection.py`, trace-capture,
> `reflect()` + shadow-записи, `migrate`/`rules` CLI). Основний план описує, як
> правила **народжуються** (human override + self-reflection) і як вони
> **застосовуються** (фази 6–8). Цей документ описує ортогональний зріз: як
> людина **бачить, аудитує і редагує** те, що накопичилось, і як інтерфейс
> Astryx переходить на React.
>
