...
Додай `bin/ui_verify.sh`, що перевіряє INV-1..INV-3 і INV-4 (через playwright), повертає != 0 при порушенні, і підключи його до `bin/sdd_verify.sh` та CI. **Спочатку скрипт (він має падати), потім код.**
