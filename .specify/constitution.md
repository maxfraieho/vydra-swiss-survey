...
## 3.1. Критерій ADR-значущості (додано 022-adr-integration, 2026-08-18)
...
`docs/adr/template.md`) — див. `docs/adr/0002-madr-format-adoption.md`.
ADR — джерело кандидатів для майбутніх інваріантів цієї конституції, сам
по собі НЕ новий шар пріоритету: конституція лишається найвищим шаром
(конституція > методологія > ADR-обґрунтування > транспорт).
...
1. Впливає на нефункціональні характеристики (безпека, продуктивність,
   стабільність з'єднання чи детермінізм hot path).
...
4. Змінює межі/контракт між підсистемами (напр. Flask API ↔ browser
   agent, формат `rule_applications`).
...
контракту) — лишається в `spec.md`, ADR не обов'язковий. Перевірка
чекліста виконується на Plan-кроці Шаблонів 1/3/4 методики (`docs/for-
agents/sdd-development-methodology.md`) ПЕРЕД переходом у `tasks.md`.
